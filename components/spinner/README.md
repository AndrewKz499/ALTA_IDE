...
▶ [Открыть живой Preview](https://andrewkz499.github.io/ALTA_IDE/components/spinner/preview.html)

## Назначение

Веб-реализация компонента **Spinner** из AltaIDE DS. Компонент использует единую DOM-структуру для Light/Dark и семантические токены из `tokens/tokens.css`.

## Файлы

- `index.html` — канонический пример разметки;
- `spinner.css` — стили компонента;
