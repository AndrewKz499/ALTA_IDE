...
▶ [Открыть живой Preview](https://andrewkz499.github.io/ALTA_IDE/components/bit-mask-picker/preview.html)

## Назначение

Веб-реализация компонента **Bit Mask Picker** из AltaIDE DS. Компонент использует единую DOM-структуру для Light/Dark и семантические токены из `tokens/tokens.css`.

## Файлы

- `index.html` — канонический пример разметки;
- `bit-mask-picker.css` — стили компонента;
